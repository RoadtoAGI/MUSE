...
1. **先处理手选作品**：当前 phase 学习面命中某个 `prefer` 项的 `intended_domains` 时，精确限定到该作品；该作品承担对应领域的来源事实，自动候选只补充未覆盖领域。phase=2 且指定固定人物时，完整读取该人物档案或角色 Skill，先看清全弧再按 `story_timepoint` 切开；随后只沿 locator 读取与切片前承重经历、当前决定、声音或盲区直接相关的原作场景。相关证据充分时停止，不为形式完整通读全书
...
1. **选中参考作品**（含书名、匹配的设计尺度与结构问题）
2. **每个参考作品的 phase-aligned 设计摘要**：
   - 来源事实（世界规则、人物或结构在原作中实际是什么，并给出原文件定位）
   - 保留项与可改项（哪些来源事实须进入本篇因果，哪些承载方式由 writer 自主选择）
   - 结构组织（字段间的依赖 / 因果链 / 对位）
...
6. **直接复用候选**：按项目适配度列出可直接复用或复制的人物 / 设定 / 情节 / 专名 / 原词 / 原句 / 连续段落；跨语言候选可按交付语言翻译、转写或保留原文
