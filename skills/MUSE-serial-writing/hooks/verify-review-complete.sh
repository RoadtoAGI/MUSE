...
# 拦截全文拼接入口：scripts/assemble_story.py <work_dir>
# 缺失 review/scene_*.yaml 且无 escape hatch → exit 2 阻断
# assemble_story.py 命令明确出现但 work_dir 无法解析 / 不存在 → exit 2 阻断
#
# 协议：serial-chapter-writing/references/execution-protocol.md §1.5
...
if [ ! -f "$SCRIPT_DIR/verify_review_complete.py" ]; then
  echo "[verify-review-complete WARN] verify_review_complete.py 未找到（不阻断）" >&2
  exit 0
fi
