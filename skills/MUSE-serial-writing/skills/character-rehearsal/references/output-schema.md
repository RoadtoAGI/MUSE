...
moves:
  - "on": cue_1
    ...
    move: "<本角色会实施的一次台词、动作、沉默或自然组合>"
    intended_effect: "<角色希望对方或现场发生的即时变化>"
```
...
一次选择不再拆成独立的心理、决策、动作、台词和反应栏目。`meaning` 保留简短人物依据，`move` 提供候选表面，`intended_effect` 提供场内目的；三者各有用途，不重复陈述同一内容。
