...
思想或叙事问题的研究保留 mechanisms：

```yaml
mechanisms:
  - creative_move: 原作的具体构想
    narrative_reason: 动机、情境、铺垫与后果如何支持这一解释
    mechanism: 可重建的关系或因果结构
    transfer_conditions: 当前迁移须具备的条件及失效位置
    source_ids: [SRC-001]
```

mechanisms 按实际问题填写，不要求每卡完整覆盖。source_ids 回到本卡 sources，作者自述有独立来源时单独说明。
...
`source.verified_by` 写 `inspiration-research`。canon 缺少稳定卡 ID 时以作品、PROT-* 与实际原文定位追溯；web / direct 不制造 kb_id。采用阶段补齐 `type`、状态和相应设计绑定；只有分阶段揭示的机制需要 `disclosure_ladder`。writer 从绑定的 INS-* 取得实际内容，候选路径本身不代替上下文。

连载的 ledger 由全系列共享。采用者为 Phase 5 的 project_encoding 写实际 chapter_id 与本章局部 scene_id；同号 S01 跨章不可互用。disclosure_ladder 若按场定位，同样保留章身份；缺少旧章绑定时由设计负责人确认后补齐本次采用，不用最新章号猜测。
