...
用法：
    python3 ${CLAUDE_PLUGIN_ROOT}/scripts/extract_scene_card.py --scene-id S02 --work-dir <含 pipeline/ 的 run 根目录>
...
def render_scene_task(task) -> str:
    """把 r10 scene_task 投影成 writer 可读语义，不暴露内部字段名。"""
    if not isinstance(task, dict):
        raise SchemaError(
            f"scene_task 必须是 r10 object，got {type(task).__name__}: {str(task)[:80]!r}"
            "（full 链不消费字符串任务，请先跑 validate_phase5_r10.py）"
        )

    lines: list[str] = []
    abstract_function = task.get("abstract_function")
    if abstract_function:
        lines.append(f"- **需成立的叙事工作**: {abstract_function}")

    physical_carrier = task.get("physical_carrier") or []
    if physical_carrier:
        lines.append("- **候选承载（可替换、合并或舍弃）**:")
    ...
    if reader_yield:
        lines.append(f"- **目标叙事增量**: {' / '.join(str(item) for item in reader_yield)}")
    ...
    if rendering:
        ...
        if expand_condition:
            lines.append(f"- **呈现建议（不规定正文顺序）**: {mode_label}；仅在{expand_condition}时展开")
        else:
            lines.append(f"- **呈现建议（不规定正文顺序）**: {mode_label}")
...
def render_scene_card_markdown(scene: dict) -> str:
    ...
    lines.append("## 可用创作材料")
    lines.append("")
    lines.append("以下条目是可替换素材；输入顺序不代表正文顺序。只复述同一结果的内容应合并；重复本身产生新效果或承担来源复用时保留。")
    lines.append("")
    ...
    if inspiration_refs:
        ...
        lines.append("")
        lines.append("本场采用 `pipeline/inspiration_ledger.yaml` 中以下 INS-* 机制；卡内实际存在 disclosure_ladder 时再消费对应层：")
        lines.append("")
...
def _render_prose_risk_contract(scene: dict, lines: list[str]) -> None:
    ...
    - 对象缺失 OR used=False → 整段不输出；已声明对象的格式无效时报 SchemaError
    ...
    if contract["used"] is not True:
        return
    risk_families = values["risk_families"]
    positive_strategy = values["positive_strategy"]
    bad_shape_examples = values["bad_shape_examples"]
...
def _render_world_disclosure_plan(scene: dict, lines: list[str]) -> None:
    ...
    if forbid:
        lines.append("**暂缓披露**（当前 POV 不可知，或延迟后产生明确收益）：")
        lines.append("")
        for item in forbid:
            lines.append(f"- {item}")
        lines.append("")
    ...
    if allow:
        lines.append("**本场可披露**（服务场景需要的认识、因果、关系或表达）：")
        lines.append("")
        for item in allow:
            lines.append(f"- {item}")
        lines.append("")
...
def main() -> int:
    ...
    parser.add_argument("--work-dir", required=True, type=Path, help="run 根目录（其下含 pipeline/）")
