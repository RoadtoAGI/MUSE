...
def validate_post_revision_pass(
    ...
    from ai_policy import RULE_TO_FAMILY, density_contract_max_count, effective_policy

    hits = [hit for hit in lint_v2.get("hits", []) or [] if isinstance(hit, dict)]
    lang = lint_v2.get("language", "zh")
    units = (lint_v2.get("density") or {}).get("total_chars")
    if isinstance(units, int) and units > 0:
        for family in {hit.get("family") or RULE_TO_FAMILY.get(hit.get("rule")) for hit in hits} - {None}:
            policy = effective_policy(family, lang)
            maximum = density_contract_max_count(policy, units)
            family_hits = [hit for hit in hits
            ...
            if maximum is not None and len(family_hits) > maximum:
                required_hits.update(hit.get("lint_id") for hit in family_hits if hit.get("lint_id"))
