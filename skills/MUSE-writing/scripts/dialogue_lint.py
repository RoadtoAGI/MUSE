...
def main() -> int:
    ...
    parser.add_argument("--story", type=Path, default=None,
                        help="全文模式：直接 lint 整篇正文（如 story.md），输出 whole.dialogue.yaml（scene_id=WHOLE）")
    ...
    if bool(args.scene_id) == bool(args.story):
        print("[dialogue_lint] ERROR: --scene-id 与 --story 二选一", file=sys.stderr)
        return 1
    ...
    if args.story:
        ...
        out_name = "whole.dialogue.yaml"
    else:
        scene_id = args.scene_id
        scene_path = work_dir / "pipeline" / "scenes" / f"scene_{scene_id}.md"
        out_name = f"{scene_id}.dialogue.yaml"
