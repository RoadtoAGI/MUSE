...
真实多线且 Phase 5 需要定位时使用 `narrative_threads`，记录 `line_id/description/sequence_refs`；跨线影响写进已有序列结果或因果链。按[结构参考](references/mckee-structure.md)区分主题矛盾、主题回响、启动铺垫和制造纠葛，也可通过同一人物的两种牵挂形成联系。切换时考虑离开该线后保留的关切，以及进入另一线会取得的进展或理解；单线省略该块。
