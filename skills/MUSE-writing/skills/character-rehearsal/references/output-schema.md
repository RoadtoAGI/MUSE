...
moves:
  - "on": cue_1  # 可选：仅在回应 observable cue 时写；主动发起时省略
    ...
    move: <角色的一次台词、动作、沉默，或自然组合>
    intended_effect: <角色想让对方或现场发生的即时变化>
```
...
2. **一个选择只写一次**：台词、动作、沉默与身体反应可以在同一 `move` 中自然组合，不把同一选择拆成 decision / action / line / reaction / tell 多栏复述。
...
6. **效果属于角色**：`intended_effect` 写角色想在场内做成什么，不写作者对人物弧光、潜意识或主题的解释。
7. **候选保持稀疏**：只有能够改变 writer 对角色行动或对白实现的选择时才写；普通补全、同义改写和无独特人物依据的反应省略。候选数量由独立选择决定，不设固定上限或最低数量。
