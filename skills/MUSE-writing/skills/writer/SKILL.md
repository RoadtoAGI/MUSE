...
10. 当前派发明确给出的有效 ref 路径（通常为 `pipeline/references/{scene_id}_ref.md`；未给出或明示“无”时跳过，目录中的旧文件没有输入权。调度见 `phase6-scene-development/references/execution-protocol.md §3.5`）
	   - **读取时机**：其余输入全部消化后、动笔前**最后**读——ref 是文风锚，离生成越近引力越强
	   - **style anchor 提取**（内隐不落盘）：从 ref 原文提炼本场真正有用的文风锚点；可关注叙事语态、句段呼吸、词汇质地、对白形态或留白方式，按场景选择，不设数量和覆盖配额
	   - **paired function bridge**：ref 中存在 `<function_bridge>` 时，用其中“目标场景职责—源场景职责线索—迁移边界”理解原文怎样改变危险、选择、关系或读者判断；复用力度仍由 `reuse_tier` 决定。该块缺失时直接读取原文，不自行补造配对分析
	   - **正向复用**：reference 贴切时，优先直接复用或复制其中的人物、设定、情节、专有名词、原词、原句与连续段落，不设字符或段落长度上限；与当前设计契约冲突时，以当前设计为准。裁量服从原文。
	   - 外部作品原文与内部 canon reference 均适用。
	   - **复用指令（按 ref 头部 `reuse_tier` 分档）**：复用是本场要求，非可选——
	     - full 档：复用 ref 原文中贴切的专名、原词、原句与连续段落，『复用候选』区块优先
	     - material 档：只复用与本场直接相关的来源专名、世界事实和专门术语；低功能同构范文的通用动作、物件和单句不构成复用义务
	     - 两档回执附复用清单；空清单仅限无候选或候选均冲突故事不变量
	   - **最小接合式语态归一**：仅改人物 / POV / 时态 / 指代 / 专名 / 衔接，保留原文措辞、句法与段落质地
	   - **素材的作用**：根据当前情境判断 lore、复用材料与声音如何服务行动因果、人物认识、关系、读者定向或表达效果；无相应作用的内容删减。材料可以通过动作、感知、对白或叙述实现，不要求每条规则绑定动作。
	     - 角色声音依据人物经历、长期声音与当前压力实现；风险提示只作关注点。
	     - `abstract_function`、`reader_yield` 等作者侧说明用于理解场景目的，正文用既定叙述方式实现；“读者从行为推断”的内容保留推断空间。
	   - **世界观复用**（有效 ref 含 `worldview_reuse`）：读取 `<worldview_lore>`，保留已选运行机制与适用条件。人物只使用其确有渠道获得的信息；人物未知的确定事实可暂缓或按既定叙述视角呈现，不自动改成传闻。来源本身不确定时保留不确定性。回执沿用来源条目与正文落点说明。
	   - **与 prose_risk_contract 关系**：contract 只提供风险关注，不因复用 ref 原文降低力度；复用段落同样语态归一
	   - ref 内标注"仅文风参考"的条目只贴文风；style anchor 与设计文档冲突时设计优先
	   - **跨语言降级**：ref 语言与产出语言不一致时只贴语态 / 节奏 / 留白
	   - 文件不存在时跳过，不报错（扩展包未装 / 场景不命中判据 / KB 缺失等统一降级）
...
| 名著原文复用契约 | ref 的 `reuse_tier: full` 所授权的贴切原词、原句、连续段落，以及 `material` 档与本场直接相关的来源专名、世界事实和专门术语 | 在故事不变量和功能适配范围内最大程度复用；仅做接入本篇所需的最小调整，优先级高于 writer 的措辞、句式与文风偏好 |
| 场景实现素材 | scene_tasks / role moves 的具体动作、物件、局部顺序、对白候选与渲染详略 | 按阅读效果改写、合并、替换、调序或舍弃；只保留能完成场景功能的部分 |

- 设计材料规定需要成为真的事实、因果与效果；正文按人物经历事件的方式组织。多个输入承担同一叙事增量时，只合并对同一结果的复述；重复继续改变行动、关系、危险、理解、不可逆后果、形式效果或承担来源复用时保留。
...
- 大纲规定本场产生的变化，正文决定变化怎样发生。相同 `reader_yield` 的动作步骤只在复述同一结果时合并；有效复沓与来源文本要求的重复继续成立。
- 用户或上游提到某次普通操作，只保证相应故事事实的因果与语义结果成立，不自动要求把操作步骤写给读者。清单只用来说明“已经整备”或某项人物特质时，具体名词也不构成逐字保留义务；没有后续作用的条目不进入结果句。只有用户明确要求这些对象逐一出现在成品中，或对象身份实际改变本场时，才保留具体名称。用户明确要求描写某个过程时，选择真正推动该过程的判断、阻力与后果展开；列举的物件和动作仍按当前作用取舍，不能把题面名词逐项兑换成正文句子。
...
关键行动应能从角色已有前提、当前刺激与可行代价推出，并对危险、资源、关系、选择空间或理解造成可观察后果。人物可以误判、自欺、偏执或犹豫；其前提、解释和行动仍需连续。

`role_move.meaning` 只帮助理解候选行动的角色因果。正文通过人物所见、所做、所说及其后果呈现该因果，不把 `meaning` 改写成旁白讲解。候选对白和动作只有在贴合当前场景时才进入正文；场景节奏、叙述次序与最终措辞由 writer 决定。

## Craft Preflight（写正文前完成，不输出）

结合核心冲突、入场处境、离场结果、阅读焦点和可用材料，决定正文从哪一刻进入、由谁感知、信息按什么次序出现、哪些过程省略。存在 `style_directives` 时以其约束作品整体声腔；存在 `craft_targets` 时，从中选择真正适合本场的承载、省略、尺度、人物塑造或叙述位置策略，不要求逐项使用，也不把字段说明写进正文。具体手法由场景压力与全文节奏产生；线性写法有效时直接使用，相似组织已使本场作用钝化时再调整入口、时间关系、观察角度或收尾。`craft_targets` 与 `craft_carrier` 提供候选实现方向，不构成数量要求或模板；它们不覆盖故事不变量与名著原文复用契约。
...
writer 可采用、组合、改造或舍弃这些机制，也可选择表外方法。`primary / secondary` 只表示参考重心；`forbidden_moves` 只有在承载故事事实、人物知识边界、核心因果或 handoff 时才是硬约束，其余内容按风险提示处理。高潮成品验收关注认知转折、因果兑现、不可逆后果与代价，不检查拍数、比喻数、主语词类或固定措辞。
...
| `目标叙事增量` | 判断句子是否值得存在的收益锚 |
| `呈现建议（不规定正文顺序）` | 决定给结果、给锚点或展开过程 |

**使用判据**：
阅读焦点、候选承载、目标叙事增量与呈现建议共同约束成品效果；writer 可选择、改造或替换材料，并按场景需要写、合并、省略或展开。材料之间没有逐项交付关系。

历史 `[核心][main]` 字符串任务只作为 legacy fallback：按旧 reader_layer 规则消费；新 scene_card 使用对象任务的 writer-facing 投影。
...
scene_card 的阅读焦点确定本场跟随线，呈现建议帮助选择详略。writer 让读者亲历最有戏剧代价的变化，其余信息可以给结果、压缩、后置或省略。连续出现物证、规则或判断时，用行动、人物压力或因果变化承接，避免把大纲任务平铺成同等密度的正文。

让人物按场内因果出现；一次选择可以同时显出多人的差异，无需轮流展示角色卡。额外判断只有继续改变行动、关系或压力时才进入正文。信息通过人物压力、行动选择和情绪变化抵达读者，逻辑链与行动链保持同一条因果。

**省略是默认值**：读者能自行补完的移动、检查、整理、等待、转身、开门、下楼，跳过或压成结果。动作过程只有在其次序或方式使读者亲历局部逆转、人物选择、不可逆代价、世界规则或来源文本效果时才展开。

**角色特质 ≠ 叙述形态**：角色可以清单化思考（程序员、PM、军官等 POV 常见），但正文不能清单化交付。读者要读到人物在压力中的选择，不是读到操作日志。

**潜在收益不替动作自证**：scene card 或任务背景写“这样以后更快 / 更安全 / 更专业”，而本场没有让该方式实际改变信息、危险、资源、关系或选择时，给出整备后的结果或直接省略。不要追加解释句，也不要临时增设事故来兑现候选素材。例如“大夫把止血带、剪刀、纱布逐项放好，因为这样取用更快”仍是操作报账；本场若只需要他已准备妥当，写到结果即止。若突发出血已经属于本场核心因果，器材位置确实改变抢救窗口，才让读者经历必要的取用次序。

压成一句仍可能是操作报账。负例：“演出前，他打开琴盒，把备用弦、松香和谱夹逐一看过，确认无误。”改成“琴盒已经收好，备用弦、松香和谱夹各在原位”仍在结算清单。若整备事实需要衔接，写结果：“琴已经调好，他在幕边等报幕。”若连整备结果也不影响此刻阅读，直接从报幕或演奏进入。人物的专业性可由随后处理真正困难的方式显出。
...
交稿前执行一次语义删并测试：删除该动作或把它并入前后句后，读者是否会失去一项本场已经发生的具体变化——因果结果、危险或资源状态、人物选择、关系或世界认知、当前压力的变化，或与来源文本绑定的有效形式效果。答案为空时删除或合并；答案存在时，正文需要让该变化可感知。候选动作只有潜在用途时，不能用解释或新增情节把答案改成“存在”。

“渲染紧张”“控制节奏”“体现类型质感”是待证实的效果。动作需要实际造成或显出声音暴露、留下踪迹、退路封闭、伤势恶化、时间或资源损失、误判形成、选择空间收窄等变化，才能用这些理由展开。职业身份、日志 / 清单 / 碎段 / 短切装置、scene_card 的 `literary_device` 与 `prose_risk_contract` 都不自动豁免信息有效性和 AI pattern 判断。短句、重复或停顿仍可在具体语境中成立。

## 产出约束

- 正文**严格不含**设计 token、scene_card 标题或执行过程：`[核心]` / `[灵感]` / `[惊艳]` / `scene_tasks` / `handoff` / `value_start` / `value_end` / `spine_statement` / role view 与 role move 字段名 / “需成立的叙事工作”“目标叙事增量”“呈现建议”“入场处境”“离场结果”等投影标签 / 创作理论元话语；普通故事内用词不因同形而被禁
...
- 不读 / 不写其他场景的文件
- **禁把候选清单机械倾倒进正文**：`moves` 是稀疏候选素材；逐条搬运会让场景成为行动报账。贴合当前节奏和人物因果的 move 可直接采用、改写、合并或截断，其余舍弃
