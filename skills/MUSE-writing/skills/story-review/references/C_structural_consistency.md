...
涉及一般原创秘密时，沿实际呈现顺序核对 Phase 3 `reader_spine` 与 Phase 5 的 `reader_track / scene_tasks / omission_plan`：读者当前获得了哪些事实，在哪个位置确认答案，当时的延迟条件是否仍生效，关键分晓是否保留前文事实并形成所需认识或后果。引用具体正文与对应设计条件，沿现有 `pipeline_crosscheck` 报告；与 A 组已定位的披露问题归并。世界规则和实际采用的来源 ladder 各按既有约束核对，无相应设计时不补造保密要求。

人物知识按经历与获知时点另行判断。读者先知而人物未知、先告结果再追原因、误解经新证据得到修正均可成立；这些结构需要保留各自的理解依据。解释出现的篇幅与候选计划不同，本身不足以判定偏离。
