...
**最新进展 · muse-runtime v0.1.0（2026-09-23）：JEV 辅助选材和剧情构思，让作者反馈进入下一轮共创。**
...
[JEV 辅助创作](#jev-如何参与创作) · [理论基础](#麦基理论如何指导创作) · [名著知识库](#从名著中学习写法) · [工作原理](#工作原理) · [技能包](#技能包) · [快速开始](#快速开始) · [仓库结构](#仓库结构)
...
**2026-09-23 更新 · muse-runtime v0.1.0**。这是论文发表后新增的创作辅助功能；版本号对应 JEV 运行组件，各技能包沿用自己的版本号。

MUSE 接入了 JEV，为参考选材和剧情构思提供结构化评价。创作模型结合评价回读材料、推演情节、解释取舍，再由作者决定采用的方向。

| 使用场景 | JEV 参与的步骤 | 如何用于创作 |
|---|---|---|
| **名著参考检索** | 在召回后读取候选场景原文或灵感卡，评价它们与当前需求、文风和使用条件的适配程度 | 查询工具按评价选择或重排参考，将选中的原文和卡片交给设计者、写作者 |
| **原创小说和剧本构思** | 在具体构思形成后比较候选，给出优先讨论、补强、重构或补充背景的建议 | 设计负责人完善人物行动和情节因果，向作者说明推荐依据与取舍 |
| **连载未来剧情讨论** | 卷向或章级 brainstorm 带入已有剧情、人物所知、固定设定及作者反馈，评价不同后续走向 | 共创流程据此展开、组合或重做候选；作者采纳后，将决定写回大纲和后续章节计划 |

例如，讨论“主角发现同伴隐瞒了真相，下一章如何推进”时，可以先展开当面揭穿、暗中求证、暂时合作等具体剧情。JEV 对照人物掌握的信息和作者希望产生的阅读效果，辅助判断哪些方向值得讨论、哪些需要补足行动依据。作者若认可“暂时合作”，同时要求保留猜疑，这条反馈会进入下一轮构思，继续发展双方的行动和关系。

JEV 按作品启用，默认使用标准模式。检索评价沿用原有参考文件，brainstorm 的评价则由构思负责人用于讨论和修订；作者的选择继续决定正式剧情。启用时，相关查询、候选材料或构思上下文会发送到所配置的 JEV 服务。

接入说明：[检索增强](skills/MUSE-canon-distill/README.md#可选-jev-检索增强) · [原创构思](skills/MUSE-writing/README.md#可选-jev-辅助构思) · [连载共创](skills/MUSE-serial-writing/README.md#可选-jev-辅助构思)。
